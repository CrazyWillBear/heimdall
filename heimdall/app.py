...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    inject a mock pool without triggering the real Redis connection.
    ...
    async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
        pool = await arq.create_pool(arq.connections.RedisSettings.from_dsn(_settings.redis_url))
    ...
    # run (e.g. in tests that patch enqueue_review and never need the real pool).
